...
// same pointers : but for average layer behaviour
std::vector<vector<double>*> av_strain_xx, av_strain_yy, av_strain_zz, av_strain_yz, av_strain_xz, av_strain_xy;
std::vector<vector<double>*> av_stress_xx, av_stress_yy, av_stress_zz, av_stress_yz, av_stress_xz, av_stress_xy;
...
// data extractor for layer behaviour
void extractdata2(vector<int> layer_box, vector<double> layer_box_vol,
            expression strain_tot, expression stress_corr,
            vector<double> thick_lays);

// save all the data in vectors into text files
void savedata(std::vector<string> tags);
...
int main(void)
{
    ...
    // average layers for trans. loading
    int av_botcu = 15, av_botag = 16, av_sub = 17;
    int av_buffer = 18, av_YBCO = 19, av_topag = 20 , av_topcu = 21;

    // av. layers as a vector
    std::vector<int> layer_box = {av_botcu, av_botag, av_sub,
       av_buffer, av_YBCO, av_topag, av_topcu};
    ...
    // volume(s)
    // av. box for cool-down and axial
    double vol = expression(1).integrate(avg_box, 4);
    // mid_box for trans.
    double vol_mid_box = expression(1).integrate(mid_box, 4);
    // layers boxes for trans.
    std::vector<double> layer_box_vol;
    for (int i=0 ; i<num_lays ; i++) {
        //
        layer_box_vol.push_back(expression(1).integrate(layer_box[i], 4));
    }
        ...
        switch(study_case) {
            ...
            case 0: {
                ...
                for (int i = 0 ; i<(steps_III+1) ; i++) {
                    ...
                    // av. layer boxes at mid
                    extractdata2(layer_box, layer_box_vol,
                                strain_elastic, stress_elastic,
                                thick_lays);
                ...
                int nx, int nz, vector<int> num_nodes_lays)
{
    ...
    // av. layers as a vector
    vector<int> layer_box = {av_botcu, av_botag, av_sub,
       av_buffer, av_YBCO, av_topag, av_topcu};
    ...
    */
    ...
    // av. layer boxes
    double th_a = 0.0 ,th_b = 0.0;

    for (int i=0 ; i<num_lays ; i++) {
        //
        th_a = th_b;
        th_b = th_b + thick_lays[i];
        mymesh.selectbox(layer_box[i], body, 3, {0.0-eps, w+eps,   th_a-eps, th_b+eps,  len/2.0-4.0e-4-eps, len/2.0+4.0e-4+eps});
    }
...
void prepvecs(int num_lays)
{
    ...
    for (int i=0; i<num_lays ; i++) {
        ...
        av_strain_xx.push_back(L_strain_xx);
        av_strain_yy.push_back(L_strain_yy);
        av_strain_zz.push_back(L_strain_zz);
        av_strain_yz.push_back(L_strain_yz);
        av_strain_xz.push_back(L_strain_xz);
        av_strain_xy.push_back(L_strain_xy);

        av_stress_xx.push_back(L_stress_xx);
        av_stress_yy.push_back(L_stress_yy);
        av_stress_zz.push_back(L_stress_zz);
        av_stress_yz.push_back(L_stress_yz);
        av_stress_xz.push_back(L_stress_xz);
        av_stress_xy.push_back(L_stress_xy);
      ...
      for (int i=0; i<num_lays ; i++) {
          ...
          av_strain_xx[i]->insert(av_strain_xx[i]->begin(), 0);
          av_strain_yy[i]->insert(av_strain_yy[i]->begin(), 0);
          av_strain_zz[i]->insert(av_strain_zz[i]->begin(), 0);
          av_strain_yz[i]->insert(av_strain_yz[i]->begin(), 0);
          av_strain_xz[i]->insert(av_strain_xz[i]->begin(), 0);
          av_strain_xy[i]->insert(av_strain_xy[i]->begin(), 0);

          av_stress_xx[i]->insert(av_stress_xx[i]->begin(), 0);
          av_stress_yy[i]->insert(av_stress_yy[i]->begin(), 0);
          av_stress_zz[i]->insert(av_stress_zz[i]->begin(), 0);
          av_stress_yz[i]->insert(av_stress_yz[i]->begin(), 0);
          av_stress_xz[i]->insert(av_stress_xz[i]->begin(), 0);
          av_stress_xy[i]->insert(av_stress_xy[i]->begin(), 0);
            ...
            vector<double> thick_lays, double w, double len)
{
    ...
    // 1)strain 2)stress 3)whole tape
            ...
            vector<double> thick_lays)
{
    ...
    for (int i=0 ; i<num_lays ; i++) {
        ...
        // strain interpolation @layer_box and pushing it back
        val = entry(0,0, strain_tot).integrate(layer_box[i], 4);
        av_strain_xx[i]->push_back(val/layer_box_vol[i]);
        val = entry(1,0, strain_tot).integrate(layer_box[i], 4);
        av_strain_yy[i]->push_back(val/layer_box_vol[i]);
        val = entry(2,0, strain_tot).integrate(layer_box[i], 4);
        av_strain_zz[i]->push_back(val/layer_box_vol[i]);
        val = entry(3,0, strain_tot).integrate(layer_box[i], 4);
        av_strain_yz[i]->push_back(val/layer_box_vol[i]);
        val = entry(4,0, strain_tot).integrate(layer_box[i], 4);
        av_strain_xz[i]->push_back(val/layer_box_vol[i]);
        val = entry(5,0, strain_tot).integrate(layer_box[i], 4);
        av_strain_xy[i]->push_back(val/layer_box_vol[i]);

        // stress interpolation @layer_box and pushing it back
        val = entry(0,0, stress_corr).integrate(layer_box[i], 4);
        av_stress_xx[i]->push_back(val/layer_box_vol[i]);
        val = entry(1,0, stress_corr).integrate(layer_box[i], 4);
        av_stress_yy[i]->push_back(val/layer_box_vol[i]);
        val = entry(2,0, stress_corr).integrate(layer_box[i], 4);
        av_stress_zz[i]->push_back(val/layer_box_vol[i]);
        val = entry(3,0, stress_corr).integrate(layer_box[i], 4);
        av_stress_yz[i]->push_back(val/layer_box_vol[i]);
        val = entry(4,0, stress_corr).integrate(layer_box[i], 4);
        av_stress_xz[i]->push_back(val/layer_box_vol[i]);
        val = entry(5,0, stress_corr).integrate(layer_box[i], 4);
        av_stress_xy[i]->push_back(val/layer_box_vol[i]);
...
void savedata(std::vector<string> tags)
{
    ...
    for (int i=0; i<tags.size() ; i++) {
        ...
        // writing strain data in txt files
        writevector("./matlab_folder/results/av_" + tags[i] + "_strain_xx.txt", *av_strain_xx[i], '\n');
        writevector("./matlab_folder/results/av_" + tags[i] + "_strain_yy.txt", *av_strain_yy[i], '\n');
        writevector("./matlab_folder/results/av_" + tags[i] + "_strain_zz.txt", *av_strain_zz[i], '\n');
        writevector("./matlab_folder/results/av_" + tags[i] + "_strain_yz.txt", *av_strain_yz[i], '\n');
        writevector("./matlab_folder/results/av_" + tags[i] + "_strain_xz.txt", *av_strain_xz[i], '\n');
        writevector("./matlab_folder/results/av_" + tags[i] + "_strain_xy.txt", *av_strain_xy[i], '\n');

        // writing stress data in txt files
        writevector("./matlab_folder/results/av_" + tags[i] + "_stress_xx.txt", *av_stress_xx[i], '\n');
        writevector("./matlab_folder/results/av_" + tags[i] + "_stress_yy.txt", *av_stress_yy[i], '\n');
        writevector("./matlab_folder/results/av_" + tags[i] + "_stress_zz.txt", *av_stress_zz[i], '\n');
        writevector("./matlab_folder/results/av_" + tags[i] + "_stress_yz.txt", *av_stress_yz[i], '\n');
        writevector("./matlab_folder/results/av_" + tags[i] + "_stress_xz.txt", *av_stress_xz[i], '\n');
        writevector("./matlab_folder/results/av_" + tags[i] + "_stress_xy.txt", *av_stress_xy[i], '\n');
